...
const FilterTimeContainer = tw.div`
  h-14 flex-col justify-between py-4
`;
...
const FilterTimeBtn = styled.button((props: { selected: string }) => [
  tw`w-[128px] h-9 flex rounded-full justify-between items-center pl-[14px] pr-[10px] py-4 border-[1px] border-primary`,
  props.selected === '' ? tw`bg-black` : tw`bg-primary`,
]);

const FilterTimeHolder = styled.span((props: { selected: string }) => [
  tw`text-tiny-bold truncate w-18 text-start`,
  props.selected === '' ? tw`text-primary` : tw`text-black`,
]);

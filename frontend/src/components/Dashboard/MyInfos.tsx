...
const TextBox = tw.div`w-[80%] h-14 p-4 bg-grey20 rounded-10 text-main text-primary`;
...
const Info = ({ title, content }: InfoProps) => {
  return (
    <InfoContainer>
      <TitleContainer>{title && <Span>{title}</Span>}</TitleContainer>
    ...
    <>
      ...
      {functionToggle.interestToggle &&
        ...
        myInfo.interests !== undefined && (
          <Info
            title="관심사"
            content={`${myInfo?.interests[0]}
        ${
          myInfo?.interests.length > 1 ? `등 ${myInfo?.interests.length}개` : ''
        }`}
          />
        )}
    </>

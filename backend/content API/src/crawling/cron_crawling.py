...
def get_content():
    ...
    options.add_experimental_option("detach", False)  # to keep browser open
    ...
    for li in li_tags:
        ...
        if created_date < date.today().strftime('%Y-%m-%d'):
            break

        content_list.append(f'{url},{title},{credit_at},{created_date},{categories}')
    return content_list
